...
# Data
data<-read.table("femurbirds.txt", header=T)

# Tree
tree<-read.nexus("untitled1.trees.trees.nex")
...
data<-read.table("femurbirds.txt", header=T) # Reload data to plot it with log10 conversion
...
plot(NA,ylim=c(1L,length(tree2$tip.label)),xlim=lab,axes=FALSE,xlab="RMR (mLO2 h-1 g-0.96)")
lab <- lab[1L]:lab[2L]
